...
## Update — PRD 0007 (2026-05-17)

The user-facing REST surface specified in §2 is now implemented (`internal/transport/rest/`) and wired into `cmd/vault/main.go` on the same `:8080` HTTP listener that already served `/healthz` + `/readyz`. Five `Handler` methods (create / list / getByID / replace / delete) delegate to the five `vault.Service` methods landed in PRD 0005. Trial-mode auth is the `X-User-Id` header, exactly per §2.

Behavior and discipline notes worth pinning:

- **Sentinel→HTTP status table is implemented in `rest/errors.go`** as the symmetric counterpart to the gRPC `mapStatus` from PRD 0006. The mapping matches the table in the PRD verbatim (404 / 409 / 412 / 415 / 400 / 413 / 500) plus two additions surfaced by the in-flight PRD 0008 work: `vault.ErrProviderUnknown` → 400 ("unknown provider") and `vault.ErrProviderValidation` → 422 ("provider validation failed", with the provider-side reason surfaced in an `ErrorResponse.Reason` field for trial-mode debugging).
- **`ErrorResponse` shape was extended to `{error, reason?}`** (linter-driven during this PRD). Status code carries semantic; `error` carries category; `reason` carries provider-side detail when (and only when) the failure originates outside the vault. Body never includes row IDs, SQL fragments, or claim contents.
- **JSON list response is wrapped, not bare.** `GET /v1/credentials` returns `{"credentials":[...]}` rather than a bare JSON array. Rationale: future pagination fields (`next_cursor`, `total`) can land without breaking clients. §2's table showed a bare array as illustrative shorthand; the implemented shape is the wrapped object documented in PRD 0007's "Implementation Decisions" snippet.
- **No router library.** `net/http.ServeMux` with Go 1.22+ pattern matching (`"POST /v1/credentials"`, `"GET /v1/credentials/{id}"`). Five routes; stdlib is the boring correct answer. Zero new module dependencies for the REST surface itself.
- **`Service` signature changed mid-PRD.** PRD 0007 was authored expecting `NewService(store, tenants, cryptor, verifier)`; in-flight PRD 0008 work added a `ProviderLookup` parameter to enable `Provider.Validate` at create / replace time. All callers in this PRD pass either a strict `vault.NewProviderRegistry` (production `cmd/vault`) or the catch-all `providertest.PermissiveRegistry` (tests that do not exercise validation semantics). The strict registry has one provider registered at this writing (`s3`) — `cmd/vault/main.go` adds future providers with one `Register` call each.
- **REST contract suite lives at `internal/transport/rest/restcontract/`, not under `servicecontract/`.** PRD 0007's "Testing Decisions" originally proposed extending `internal/vault/servicecontract/` with REST scenarios, but the existing import rule `internal/vault ↛ internal/transport` (enforced by `make check-imports`) blocks that — the contract package needs to import `internal/transport/rest` to drive the handler. Moving it under `internal/transport/rest/restcontract/` preserves the same reference-contract semantics (memstore + Postgres factories drive identical scenarios) without crossing the hexagonal boundary. Both `internal/adapters/memstore/rest_test.go` and `internal/adapters/postgres/rest_integration_test.go` consume it.
- **`make check-imports` was already enforcing `internal/transport ↛ internal/adapters`** before this PRD landed. PRD 0007's "Implementation Decisions" claimed the rule needed to be added; the Makefile already had it (the original PRD 0006 wiring covered the broader `internal/transport` directory, not just `grpc`). No Makefile change was needed.
- **`http.Server` got explicit timeouts** (`ReadHeaderTimeout: 5s`, `ReadTimeout: 10s`, `WriteTimeout: 10s`, `IdleTimeout: 60s`) as part of the REST integration. Prior to this PRD the listener ran with stdlib defaults (no read deadline → slow-loris exposure). These values match the conservative trial profile; production tuning happens in the deployment PRD.
- **`POST/PUT` enforces `Content-Type: application/json`** (with charset suffix tolerated). Missing or wrong content-type returns 415 Unsupported Media Type. Bodies are capped at 1 MiB via `http.MaxBytesReader`; exceeded → 413.
- **No `Idempotency-Key`, no rate limiting, no JWT.** §4 deliberately deferred per the PRD's out-of-scope list — those join the production gateway PRD when Redis lands.
- **Demo path**: `examples/curl-demo.sh` exercises the lifecycle (POST → GET list → GET id → PUT → GET → DELETE → 404) against a live `make run` binary. Not gated by `make check`; it's the human-run smoke that confirms the trial story works end-to-end.
